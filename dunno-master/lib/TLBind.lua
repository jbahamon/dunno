--- TLbind v1.2, a simple system for creating professional control schemes
...
local function giveInstance( binds )
	...
	-- @field deadzone clip analogue input to 0 when under this number (must be >= 0 and < 1)
	...
	-- @field circleAnalogue [index#] = {"analogue 1", "analogue 2"}
	local b = {
...
update = function(b)
	...
	if b.useJoystick then
		...
		for j,binds in pairs(b.joyAxes) do for k,v in pairs(binds) do
			local x = lj.getAxis(j,k)
			control[v] = math.abs(x)>b.deadzone and x or control[v] or 0
		end end
	...
	-- Constrain analogue pairs to a unit circle (fixes "running diagonally is faster")
	for k,v in ipairs(b.circleAnalogue) do
		local x,y = control[v[1]], control[v[2]]
		if x and y then
			local l = (x*x+y*y)^.5
			if l > 1 then control[v[1]], control[v[2]] = x/l, y/l end
		else
			print("TLbind can't constrain unbound analogues! (circleAnalogue."..k.." = { "..v[1]..", "..v[2].." }?)")
			control[v[1]], control[v[2]] = 0, 0
		end
	end
end
